/**
 * NOTE - This is a duplicate https://github.com/trayio/babel-plugin-webpack-alias/blob/master/src/index.js
 * The difference being, I allow for more than just replacing Strings in `require`
 * calls by passing an Array option for `callers`. Then any method with an
 * alias for it's first arg, will have it replaced with the proper value.
 */

const { join, resolve, relative, isAbsolute, dirname, basename, sep } = require('path');
const fs = require('fs');
...
module.exports = function webpackAliases({ types: t }) {
  return {
    visitor: {
      CallExpression(path, { file: { opts: { filename } }, opts: {
        callers = [],
        config: configPath,
        ...
        if(Array.isArray(conf)) {
          ...
          if(!Object.keys(aliasConf).length) return;
        ...
        // Exit if it's not an accepted callee statement
        let callees = ['require'];

        if(callers.length) callees = callees.concat(callers);

        if(
          !callees.includes(calleeName)
          || !args.length
        ...
        for(const aliasFrom in aliasConf) {
          if(aliasConf.hasOwnProperty(aliasFrom)) {
            ...
            if(regex.test(filePath)) {
              ...
              // maintain any extra args
              if(callers.length && args.length > 1){
                args.shift();
                path.node.arguments = path.node.arguments.concat(args);
              }
